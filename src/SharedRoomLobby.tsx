...
type CommandKind = "reconnect" | "leave" | "kick" | "start" | "close" | "game_start" | "game_answer" | "game_reveal" | "anonymous_submit" | "anonymous_moderate" | "game_vote" | "game_phase" | "werewolf_action";
...
export function SharedRoomLobby({ apiUrl, onPresenceChange }: { apiUrl: string; onPresenceChange?: (hasPresence: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    socket.on("v2:projection", (next: SharedProjection) => {
      ...
      const needsPrivateRefresh = next.game?.kind === "word-wolf" || next.game?.kind === "werewolf" || (next.game?.kind === "two-choice" && !next.game.ownAnswer) || (next.game?.kind === "anonymous-box" && !next.game.ownEntry);
  ...
  async function command(kind: CommandKind, extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      if (caught instanceof Error && caught.message === "version_conflict") { try { await refresh(); } catch { /* error shown below */ } }
