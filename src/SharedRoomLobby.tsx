...
export function SharedRoomLobby({ apiUrl, onPresenceChange }: { apiUrl: string; onPresenceChange?: (hasPresence: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    socket.on("v2:projection", (next: SharedProjection) => {
      ...
      const needsPrivateRefresh = next.game?.kind === "word-wolf" || next.game?.kind === "werewolf" || (next.game?.kind === "two-choice" && !next.game.ownAnswer) || (next.game?.kind === "impression-ranking" && !next.game.ownVote) || (next.game?.kind === "anonymous-box" && !next.game.ownEntry);
