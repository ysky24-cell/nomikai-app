...
type SharedGame =
  ...
  | { kind: "legacy-game"; gameKey: string; prompt: string; mode: string; phase: "playing" | "finished"; inputCount: number; participantCount: number; ownInput?: string; currentPlayerId?: string; currentTotal?: number; targetNumber?: number; turnHistory?: Array<{ playerId: string; add: number; total: number }>; result?: { inputs: Record<string, string>; summary: string; scores: Record<string, number> } };
    ...
    <section id="shared-room-lobby" className="shared-room-lobby" aria-label="みんなのスマホで遊ぶ">
      ...
      {projection && session && (
        <div className="shared-room-waiting">
          ...
          {activeGame?.kind === "legacy-game" && <div className="shared-room-game-card"><h3>{activeGame.gameKey}</h3><p>{activeGame.prompt}</p>{activeGame.gameKey === "count-up-game" && <p className="soft-note">現在 {activeGame.currentTotal ?? 0} / 目標 {activeGame.targetNumber ?? 30}。{activeGame.currentPlayerId === session.participantId ? "あなたの番です。" : `次は${projection.participants.find((item) => item.id === activeGame.currentPlayerId)?.name ?? "次の参加者"}さんの番です。`}</p>}<p className="soft-note">入力済み {activeGame.inputCount}/{activeGame.participantCount}人。全員の入力がそろうまで結果は表示されません。</p>{activeGame.phase === "playing" ? <div className="shared-room-form"><label>あなたの回答<input value={legacyInput} onChange={(event) => setLegacyInput(event.currentTarget.value)} maxLength={500} /></label><button type="button" className="primary-button" disabled={busy || !legacyInput.trim() || (activeGame.gameKey === "count-up-game" ? activeGame.currentPlayerId !== session.participantId : Boolean(activeGame.ownInput))} onClick={() => { void command("legacy_input", { input: legacyInput.trim() }).then(() => setLegacyInput("")); }}>入力を送信</button>{isHost && activeGame.gameKey !== "count-up-game" && <button type="button" className="secondary-button" disabled={busy || activeGame.inputCount < activeGame.participantCount} onClick={() => void command("game_reveal")}>全員の結果を公開</button>}</div> : <div className="shared-room-result"><strong>結果</strong><p>{activeGame.result?.summary}</p>{Object.entries(activeGame.result?.inputs ?? {}).map(([participantId, input]) => <span key={participantId}>{projection.participants.find((item) => item.id === participantId)?.name ?? participantId}: {input}（{activeGame.result?.scores[participantId] ?? 0}）</span>)}</div>}</div>}
