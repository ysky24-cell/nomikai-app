...
```bash
curl -X POST http://localhost:3000/rooms/ROOMCD/close \
  -H "Content-Type: application/json" \
  -d "{\"participantId\":\"HOST_ID\"}"
```
...
```bash
curl "http://localhost:3000/rooms/ROOMCD/events"
```

履歴は `room_events` を元にした要約です。画面表示やAPIレスポンスでは、役職、お題、NGワードなどの秘密情報をそのまま出さない形にします。

## 現在の同期範囲

- ルーム作成、参加、観戦、接続状態表示、接続切れ表示
- 保存済みルームへの再接続、保存削除、最終同期時刻表示
- Socket.IO接続単位の参加者接続数管理。複数タブや別端末で同じ参加者が開いていても、最後の接続が切れるまで離席扱いにしません。
- ホストによるゲーム選択
- 開始、次へ、完了、待機戻し
- ホストによるルーム終了
- 終了済みルームでのゲーム開始、次へ、完了、待機戻し、ゲーム内状態更新、新規参加、ホスト交代、参加者削除の停止
- `room_events` 由来の履歴表示
- ホスト交代
- ホストによる参加者削除
- ホスト交代や削除後の参加者画面更新
...
- 人狼の役職、ワードウルフのお題、NGワードは、ルーム取得とSocket.IO配信時に参加者ごとにマスクされます
- マスク済み状態から参加者が操作しても、サーバー内の元の役職、お題、NGワードは保持されます
- 人狼、ワードウルフ、NGワードのタイマーは終了予定時刻を部屋状態に保存し、各端末が同じ基準で残り時間を表示します
- 人狼とワードウルフでは、非ホストは自分の投票順の投票だけ変更できます。最終投票後の結果はサーバー側で組み直します
- NGワードゲームでは、非ホストは踏んだ記録を1回追加する操作だけ変更できます
- 曲名連想クイズ / お絵描きクイズ / 記憶だけでロゴを書く / 変な一言カルタ / エモヒント連想 / 人物当てヒントクイズ / 鼻歌イントロドンでは、非ホストは自分の回答欄だけ変更でき、親だけが正解判定できます
- 2つの真実と1つの嘘 / 価値観メーター / スマホ早打ちゲーム / ひとこと演技ゲームでは、非ホストは自分の回答・投票欄だけ変更でき、話し手・演者だけが正解設定や結果判定を行えます
- 逆さ言葉ゲーム / 外来語禁止ゲーム / カウントアップゲーム / ドキドキはずれカード / 安全はずれ抽選では、非ホストは自分の番の操作だけ変更できます
- 飲み会すごろく / 人生イベントすごろく / シンプル陣取りでは、非ホストは自分の番のコマ移動や空きマス取得だけ変更できます
- 資源交渉トークでは、非ホストは自分の資源欄だけ変更でき、現在の交渉者だけが次の交渉者へ進めます
- 腕相撲トーナメントでは、現在の対戦カードに入っている参加者だけが勝敗を記録できます
...
- 人狼の役職、ワードウルフのお題、NGワードは参加者別にマスクして配信しますが、DB内部のルーム状態には保存されます。サーバー運用者からも見えない形の暗号化は未対応です。
- 履歴表示は `room_events` 由来の要約です。画面では秘密情報を避けますが、DB内部の `room_events.payload` や `rooms.state` には役職、お題、NGワード、回答内容などが残る場合があります。
- 本番運用では、履歴とルーム状態の保存期間、削除ポリシー、匿名化、バックアップからの削除、運用者が閲覧できる範囲を決める必要があります。
