...
- 人狼以外の各ゲーム内操作、カードめくり、回答入力はまだ端末ごとのローカル状態です。
- 人狼の役職は画面上では本人だけに表示しますが、現時点ではルーム状態JSONに含まれます。信頼できる参加者同士で遊ぶ前提です。
...
1. ワードウルフの各自お題確認と投票同期
2. 二択トーク、マジョリティの投票同期
3. ルームUX強化: ホスト交代、再接続、参加者削除、観戦者画面
4. 権限強化: ホスト操作、投票者操作、役職情報の参加者別配信
5. AWS公開用の本番Docker構成
