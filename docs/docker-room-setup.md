...
- Socket.IOの状態更新には基本的なサーバー側権限チェックがあります。ただし、上記以外の各ゲームの `votes` や `guesses` などのフィールド単位で「本人の欄だけ変更できる」ことを完全に検証する処理は今後の実装対象です。
...
1. 細粒度権限強化: 当てる系ゲーム、回答・集計系ゲーム、盤面系ゲームの回答欄や手番欄をさらに厳密に検証
2. 観戦者専用画面
3. AWS公開用の本番Docker構成
