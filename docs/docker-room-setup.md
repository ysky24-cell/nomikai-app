...
- Socket.IOの状態更新にはサーバー側権限チェックがあります。ただし、定番ゲームパック、山手線ゲーム、ウミガメのスープ、ジョハリの窓などの細かいフィールド単位検証は今後の実装対象です。
...
1. 細粒度権限強化: 定番ゲームパック、山手線ゲーム、ウミガメのスープ、ジョハリの窓の投稿欄、手番欄、ログ欄をさらに厳密に検証
2. 観戦者専用画面
3. AWS公開用の本番Docker構成
